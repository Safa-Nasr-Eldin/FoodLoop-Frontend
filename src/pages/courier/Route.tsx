 ...
 * only the two organizations, the pickup address, and where the courier stands (from status).
 */
export function Route({ task, size = 'ticket' }: Props) {
  const reduced = useReducedMotion()
  const leg = routeLegOf(task.status)
    ...
    >
      ...
      >
        <li className={cn('route__stop', leg === 0 && 'is-next', leg > 0 && 'is-done')}>
          ...
          <span className="route__name">{task.donorOrganizationName}</span>
        ...
        <li className={cn('route__stop route__stop--end', leg === 1 && 'is-next', leg === 2 && 'is-done')}>
          ...
          <span className="route__name">{task.beneficiaryOrganizationName}</span>
